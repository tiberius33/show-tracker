...
export default function ProfileView({ user, shows, userRank, onProfileUpdate, onViewShow, friends = [], favoriteArtists = [], onToggleFavoriteArtist }) {
  ...
  const handleDeleteAccount = async () => {
    ...
    // deliberate two-step act, and the ID token is what actually authorises
    // the deletion — the typing only ever added friction. It also required an
    // on-screen keyboard, which on iOS covered the confirm button and made
    // deletion impossible to complete at all. A destructive action nobody can
    // finish is worse than one that is easy to finish deliberately.
