...
- The saved session is opted into per file rather than set as a project default, because the guest-mode and sign-in tests have to start logged out. `e2e/.auth/` is gitignored — it holds a live session.
