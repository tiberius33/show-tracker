...
export function loadMusicKit() {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (window.MusicKit) {
      console.log('[Apple Music] MusicKit already available on window');
      musicKitLoaded = true;
      resolve();
      return;
    }
    ...
    const onReady = () => {
      console.log('[Apple Music] musickitloaded event fired');
    ...
    console.log('[Apple Music] Loading MusicKit JS script...');
    ...
    setTimeout(() => {
      if (!musicKitLoaded) {
        ...
        if (window.MusicKit) {
          console.log('[Apple Music] MusicKit loaded (detected via timeout check)');
          musicKitLoaded = true;
          resolve();
        } else {
...
export async function initMusicKit(developerToken) {
  ...
  console.log('[Apple Music] Configuring MusicKit...');
  console.log('[Apple Music] Developer token (first 20 chars):', developerToken.substring(0, 20) + '...');
  ...
  console.log('[Apple Music] MusicKit configured, instance:', !!music);

  // Use the returned instance or get it
  const instance = music || window.MusicKit.getInstance();
  console.log('[Apple Music] Got MusicKit instance:', !!instance);

  // Authorize (prompts Apple ID login popup)
  try {
    console.log('[Apple Music] Calling authorize()...');
    await instance.authorize();
    console.log('[Apple Music] Authorization successful');
  } catch (err) {
    console.error('[Apple Music] Authorization error:', err);
