...
function wrap(content, { uid } = {}) {
  const unsubscribeLink = uid
    ? `<p style="color:#475569;font-size:11px;margin:8px 0 0">
        <a href="${APP_URL}/api/unsubscribe?token=${encodeUnsubscribeToken(uid)}" style="color:#475569;text-decoration:underline">Unsubscribe from emails</a>
      </p>`
    : '';
...
export function showTagNotification({ taggerName, artist, venue, date, uid }) {
  ...
  const html = wrap(`
    ...
    ${button('Open MySetlists →', APP_URL)}
  `, { uid });
  return { subject, html };
}
