...
// significant absence ("bust-out"). Severity bands are fixed (90/180/365
// days) but the minimum days needed to flag at all is user-configurable —
// see DEFAULT_BUSTOUT_THRESHOLD_DAYS / BUSTOUT_THRESHOLD_OPTIONS.
//
// Data sources (no new backend — reuses what already exists):
//   - netlify/functions/get-artist-song-stats.js already fetches each
//     song's play dates across an artist's ~200 most recent setlist.fm
//     shows (same call TourInfoModal makes, shared via the same
//     `song_stats_${mbid}` localStorage cache key).
...
export const DEFAULT_BUSTOUT_THRESHOLD_DAYS = 90;
export const BUSTOUT_THRESHOLD_OPTIONS = [30, 60, 90, 180, 365];
...
// null when under threshold or unknown; otherwise 'minor' | 'major' | 'epic'.
export function getBustOutSeverity(days, thresholdDays = DEFAULT_BUSTOUT_THRESHOLD_DAYS) {
...
// that clears the bust-out threshold as of `showDate`.
//
...
//   thresholdDays         – user's configured minimum gap (days)
export function computeShowBustOuts({ setlist = [], showDate, songHistory, personalPerformances, thresholdDays = DEFAULT_BUSTOUT_THRESHOLD_DAYS }) {
  ...
  setlist.forEach(song => {
    ...
    const severity = getBustOutSeverity(days, thresholdDays);
    if (!severity) return;
