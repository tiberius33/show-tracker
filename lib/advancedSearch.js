...
// component so it's independently testable, mirroring lib/runIndex.js's
// pure-function pattern.
//
// `tourShowIds` / `festivalShowIds` are precomputed Sets of show ids (built
// by the caller — useTourIndex() for tours, a plain filter over `shows` by
// festivalId for festivals, see components/search/AdvancedSearchView.jsx)
// rather than re-deriving keys here, so a selected tour/festival always
// matches exactly the shows that lookup already decided belong to it.
