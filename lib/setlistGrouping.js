...
// ── Removing a song ───────────────────────────────────────────────────
//
// Returns { setlist, removed, index }: the array to write, the song that
// came out (so a caller can offer to put it back), and where it was in the
// ORIGINAL array (not within its set), which is what makes an undo restore
// it exactly where it was rather than at the end of its section.
//
// Unlike moveSongToSet and reorderSongWithinSet this does not round-trip
// through groupSongsBySet, because grouping infers set membership from
// array order: a song sitting between two sets would come back attached to
// a different one, and a removal must not quietly re-file its neighbours.
// A plain splice changes nothing except the song that was asked for.
//
// `removed` is null and `setlist` is the original array when the id isn't
// there, so a double-tap or a stale render cannot write an unchanged array
// over a setlist someone else has since edited.
export function removeSongFromSetlist(setlist = [], songId) {
  const list = Array.isArray(setlist) ? setlist : [];
  const index = list.findIndex(song => song?.id === songId);
  if (index === -1) return { setlist, removed: null, index: -1 };
...
// The other half of the undo: puts a song back at a given position, with
// everything it was carrying — its id, its set, its rating, its comment.
// An index past the end (or a negative one) appends rather than throwing,
// so an undo still works if the setlist shrank in the meantime.
export function insertSongAt(setlist = [], song, index) {
  const list = Array.isArray(setlist) ? setlist : [];
  if (!song) return list;

  const at = Number.isInteger(index) && index >= 0 ? Math.min(index, list.length) : list.length;
  return [...list.slice(0, at), song, ...list.slice(at)];
}
