...
export async function createSpotifyPlaylist(userId, name, description, isPublic = true) {
  // Call Spotify API directly from browser (CORS supported) to avoid proxy issues
  const accessToken = await ensureAccessToken();
  console.log('[Playlist] Creating playlist for user:', userId, '| public:', isPublic);
  console.log('[Playlist] Token (first 20 chars):', accessToken.substring(0, 20) + '...');

  // Minimal request — only required field is 'name'
  const requestBody = { name };
  if (description) requestBody.description = description;

  const url = `https://api.spotify.com/v1/users/${userId}/playlists`;
  console.log('[Playlist] URL:', url);
  console.log('[Playlist] Body:', JSON.stringify(requestBody));
  ...
  const data = await res.json();
  console.log('[Playlist] Create playlist response:', res.status, data);
