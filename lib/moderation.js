...
export async function blockUser(uid, targetUid) {
  ...
  await Promise.all([
    ...
    clearPendingBetween(uid, targetUid),
  ]);

  // Tell the admin. Deliberately last, deliberately not awaited for its
  // result, and deliberately unable to fail the block: Guideline 1.2 asks
  // that blocking notify the developer, but an account you could not
  // block because an email bounced is a far worse outcome than a
  // notification nobody received.
  notifyBlock(targetUid).catch((err) => {
    console.error('[moderation] Block notification failed:', err);
  });
}

/**
 * Drop anything still in flight between two people who have just stopped
 * speaking: a friend request either way, and any show tag one has put on
 * the other.
 *
 * Unfriending alone leaves these behind, and each of them is a way back
 * in — a pending request sits in the blocker's inbox, and a pending tag
 * puts the blocked user's name on the blocker's show.
 */
async function clearPendingBetween(uid, targetUid) {
  const jobs = [
    query(collection(db, 'friendRequests'), where('from', '==', uid), where('to', '==', targetUid)),
    query(collection(db, 'friendRequests'), where('from', '==', targetUid), where('to', '==', uid)),
    query(collection(db, 'showTags'), where('fromUid', '==', uid), where('toUid', '==', targetUid)),
    query(collection(db, 'showTags'), where('fromUid', '==', targetUid), where('toUid', '==', uid)),
  ];

  await Promise.all(jobs.map(async (q) => {
    try {
      const snap = await getDocs(q);
      await Promise.all(snap.docs.map((d) => deleteDoc(d.ref).catch(() => {})));
    } catch (err) {
      // A missing composite index on one of these must not cost the
      // other three, nor the block itself.
      console.warn('[moderation] Could not clear pending items:', err.code || err.message);
    }
  }));
}

/**
 * Record the block for the admin queue and email the developer.
 *
 * Goes through a function rather than writing `reports` directly because
 * the evidence it attaches — a snapshot of the blocked account's recent
 * posts — has to be read server-side. A client that supplies the evidence
 * supplies whatever gets someone ejected.
 */
async function notifyBlock(blockedUserId) {
  const token = await auth.currentUser?.getIdToken();
  if (!token) return;

  const res = await fetch(apiUrl('/api/notify-block'), {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  if (!res.ok) {
    const payload = await res.json().catch(() => ({}));
    throw new Error(payload.error || `notify-block returned ${res.status}`);
  }
}
