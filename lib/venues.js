...
export async function submitVerificationApplication({
  ...
  if (applicantEmail) {
    sendEmailIfAllowed({
      to: applicantEmail,
      ...venueVerificationSubmittedEmail({ venueName }),
    }, { type: 'venue_verification' }).catch(() => {});
  }
...
export async function approveVerificationApplication(application, reviewerNotes = '') {
  ...
  if (application.applicantEmail) {
    sendEmailIfAllowed({
      ...
      ...venueVerificationApprovedEmail({ venueName: application.venueName, venueKey: application.venueKey }),
    }, { type: 'venue_verification' }).catch(() => {});
  }
}
...
export async function rejectVerificationApplication(application, rejectionReason, reviewerNotes = '') {
  ...
  if (application.applicantEmail) {
    sendEmailIfAllowed({
      to: application.applicantEmail,
      ...venueVerificationRejectedEmail({ venueName: application.venueName, rejectionReason }),
    }, { type: 'venue_verification' }).catch(() => {});
  }
}
