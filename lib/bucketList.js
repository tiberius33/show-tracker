 ...
 *     }
 *
 * NOTE: this collection needs a Firestore security rule added in the
 * Firebase console before this ships — this repo doesn't contain
 * firestore.rules, so it isn't deployed from here. Mirror the `wishlists`
 * rule: allow read/write when `request.auth.uid == userId`, and allow the
 * read even when `resource == null` (first bucket-list add for a user).
 */
