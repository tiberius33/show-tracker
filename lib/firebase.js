...
// Enable Firestore offline persistence so changes made while offline or with
// interrupted connections are preserved and synced when the connection is restored.
// This is especially important for the mobile app where connection interruptions are common.
if (typeof window !== 'undefined') {
  enableIndexedDbPersistence(db).catch((err) => {
    // Persistence is already enabled or this is a secondary tab — either is fine.
    // Silently ignore the error to prevent console spam.
    if (err.code !== 'failed-precondition' && err.code !== 'unimplemented') {
      console.warn('Firestore persistence not available:', err.message);
    }
  });
}
