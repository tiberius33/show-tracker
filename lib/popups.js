...
export const POPUPS = [
  ...
  },
  {
    ...
    content: `Start by adding your first show — search for an artist and date, and we'll pull in the setlist automatically. Track your concert history, rate songs, and see stats across all your shows. Check out the How to Use guide in the sidebar for a full walkthrough.`,
    variant: 'info',
  ...
  },
  {
    ...
    targetAudience: 'all',
    newUsersOnly: false, // Critical — show to all users regardless of account age
  ...
  },
  {
    ...
    newUsersOnly: false, // Admin popups always shown
  },
];
 ...
 * @param {boolean} [opts.isReturningUser] — true if account ≥ 7 days old
 * @returns {object[]} — popup definitions the user is eligible for
 */
export function getPopupsForUser({ isAdmin = false, showCount = 0, isReturningUser = false } = {}) {
  return POPUPS.filter((popup) => {
    // Skip disabled popups (templates)
    if (popup.enabled === false) return false;

    // Account-age gating: suppress newUsersOnly popups for returning users
    if (isReturningUser && popup.newUsersOnly !== false) return false;
