...
import { doc, getDoc, setDoc, updateDoc, deleteField, serverTimestamp, collection, query, where, getDocs } from 'firebase/firestore';
...
// Returns [{ artistKey, artistName, artistMbid, songCount }, ...] for every
// artist the user has wishlisted at least one song for — the wishlist hub's
// "your artists" list. Reads only this user's own (small) wishlist docs, one
// per artist, never the external setlist.fm catalog those docs are checked
// against on the per-artist view.
export async function listWishlistedArtists(uid) {
