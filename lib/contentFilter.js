...
const WORD_SUFFIXES = [
  '', 's', 'es', 'd', 'ed', 'ing', 'er', 'ers', 'y', 'z', 'in',
  // Insult compounds. A blocked term glued to one of these is the same
  // slur with a second noun on the end — "shithead", "fuckwit",
  // "cuntface", "shitstain" — and the original list caught none of them,
  // because it only allowed inflections.
  //
  // Safe to add because the match is anchored at a TOKEN BOUNDARY: the
  // token has to START with the blocked term. "Scunthorpe" does not start
  // with "cunt" and "classic" does not start with "ass", which is why
  // those were never matches and still are not. What this changes is only
  // what may follow a term that already begins a word.
  'head', 'heads', 'face', 'faces', 'hole', 'holes', 'wit', 'wits',
  'bag', 'bags', 'wad', 'wads', 'stain', 'stains', 'tard', 'tards',
  'boy', 'boys', 'girl', 'girls',
];

// Words that legitimately begin with a blocked term and must survive it.
//
// WHY THIS EXISTS. "spic" is on the list, and the suffix allowance turned
// "spices" and "spicy" into slurs — in an app whose users are jam band
// fans, for whom "spicy" is everyday vocabulary for a good jam. That is
// the Scunthorpe problem arriving through the back door: not a substring
// match, but a prefix plus a plausible ending. Checked before the term
// patterns, so an entry here is exempt however it was reached.
...
export function findBlockedTerm(text) {
  // Blank out the exempt words first, so a term pattern cannot reach them.
  // Replaced with a space rather than deleted: removing "spicy" from
  // "a spicy jam" would leave "a jam" — fine here, but deletion is how you
  // accidentally join two neighbouring tokens into a third word.
  const blank = (value) => value
    .split(' ')
    .map((token) => (ALLOWED_WORDS.has(token) ? '' : token))
    .join(' ');

  const normalized = ` ${blank(normalize(text))} `;
  const rejoined = ` ${blank(deObfuscate(normalize(text)))} `;
